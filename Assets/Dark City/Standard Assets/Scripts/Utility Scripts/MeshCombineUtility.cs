	...
	public static Mesh Combine(MeshInstance[] combines, bool generateStrips)
	{
		...
		foreach (MeshInstance combine in combines)
		{
			if (combine.mesh)
			{
				...
				if (generateStrips)
				{
					...
					int curStripCount = combine.mesh.triangles[combine.subMeshIndex]; //combine.mesh.GetTriangleStrip(combine.subMeshIndex).Length;
		...
		foreach (MeshInstance combine in combines)
		{
			if (combine.mesh)
			{
				if (generateStrips)
				{
					int[] inputstrip = combine.mesh.triangles; //combine.mesh.GetTriangleStrip(combine.subMeshIndex);
					if (stripOffset != 0)
					{
						if ((stripOffset & 1) == 1)
						{
